...
class GeminiService{
  ...
  async generateAIPDietPlan({weight,height,age,preferences,gender,planType,unitSystem}:DietPlanReq) {
    ...
    try{
      ...
      if(!weight||!height||!age||!gender||!planType||!unitSystem||!preferences) throw new Error("All fields are required.");
        ...
        "meals": [
          {
            ...
            throw new Error("Error getting response from Gemini AI.");
          }
        }
}
